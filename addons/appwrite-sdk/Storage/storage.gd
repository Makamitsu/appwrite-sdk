...
func __post(type: int, payload: Dictionary = {}, params: Dictionary = {}) -> StorageTask:
	var temp_headers: PackedStringArray = PackedStringArray([])
...
func _read_file(file_path: String) -> PackedByteArray :
	...
	if null == file: 
		printerr("could not open ", file_path, ": ", FileAccess.get_open_error())
...
#       CLIENT & SERVER
func create_file(file_path: String, read: PackedStringArray = PackedStringArray([]), write: PackedStringArray = PackedStringArray([])) -> StorageTask:
...
func get_file_preview(file_id: String, save_path: String = "", width: int = 0, height: int = 0, gravity: String ="", quality: int = 0, border_width: int = 0, border_color: String = "", border_radius: int = 0, opacity: float = 0.0, rotation: int = 0, background: String = "", output: String = "") -> StorageTask:
	var query: String = "?"
