...
func update_password(password: String, old_password: String) -> AccountTask:
	return __post(AccountTask.Task.UPDATE_PASSWORD, { password = password, oldPassword = old_password })
...
func create_oauth2_session(provider: String, success: String = "", failure: String = "", scopes: PackedStringArray = PackedStringArray([])) -> AccountTask:
